...
/*  window.addEventListener("mouseMove" , function(evt)
  ...
  */
  ...
  _this.jumpHeight = 50; //+1 pt pour ceux qui devinent pourquoi (campagnes 2017)

  //La hauteur à atteindre( à définir quand on saute)
  _this.camera.jumpNeed = 50;
  ...
  window.addEventListener("keydown", function(evt) {
       ...
       if(evt.keyCode == 32 &&  _this.camera.canJump){
           ...
           _this.camera.playerBox.originHeight =_this.camera.playerBox.position.y;
          // console.log(_this.camera.playerBox.originHeight );
       }
   }, false);
};
...
Player.prototype = {
  ...
  _initCamera : function(scene, canvas)
  {
    ...
    */
    ...
    playerBox.position = new BABYLON.Vector3(0,150,0);
...
//console.log(this.camera.playerBox.jumpNeed);
    ...
    {
  		//TODO
    // Lerp to avoid teleportating jump
    percentMove = (this.camera.jumpNeed - this.camera.playerBox.position.y) * 0.1;
  ...
  this.camera.playerBox.position.y += upy;
   ...
   // On trace un rayon depuis le joueur
    ...
    var distPlayer = this.game.scene.pickWithRay(rayPlayer, function (item) {
        if (item.name == "Player" || item.isPlayer || item.name == "FreeCamera")
            return false;
        else
            return true;
    });
    ...
    targetHeight = this.camera.playerBox.originHeight;

    console.log("targetHeight");
      console.log(targetHeight);
        console.log("Dist au sol");
          console.log(distPlayer.distance);
          //console.log("DistPlayer");
          //  console.log(distPlayer);

    if(distPlayer.distance <= targetHeight+1){
      ...
      //TODO
      this.camera.targetHeight = this.camera.playerBox.position.y  ;
